 ...
 *
 * `centered` is for heroes with no side visual (catalogo, cotizacion): the
 * split-layout backdrop is weighted to the left to balance the widget cluster
 * on the right, which reads lopsided under centered text. This mode mirrors the
 * glow around the center axis and calms it down so the headline sits in an even
 * field instead of a heavy green wash.
 */
    ...
    <div aria-hidden className={cn("pointer-events-none absolute inset-0 -z-10 overflow-hidden", className)}>
      <div
        className={cn(
          ...
          navy
            ...
            : centered
              ? "bg-[linear-gradient(180deg,color-mix(in_oklab,var(--primary)_9%,var(--background)),var(--background)_45%)] max-sm:bg-[linear-gradient(180deg,color-mix(in_oklab,var(--primary)_7%,var(--background)),var(--background)_32%)]"
      ...
      <div
        className={cn(
          ...
          navy ? "opacity-40" : centered ? "opacity-[0.16]" : "opacity-25",
        )}
        style={{
          background:
      ...
      {centered ? (
        <div
          className={cn(
            "absolute left-1/2 top-[-32%] size-[24rem] -translate-x-1/2 animate-marketing-aurora rounded-full blur-[70px] sm:size-[32rem] lg:top-[-34%] lg:size-[38rem]",
            navy ? "opacity-70" : "opacity-30 sm:opacity-40 lg:opacity-45",
          )}
          style={{ background: "radial-gradient(circle at 50% 50%, var(--primary), transparent 70%)" }}
        />
      ) : (
        <>
          <div
      ...
      <div
        className={cn(
          "absolute bottom-[-25%] size-[32rem] animate-marketing-pulse-glow rounded-full blur-[70px] max-sm:opacity-40",
          centered ? "left-1/2 -translate-x-1/2" : "left-[26%]",
          navy ? "bg-primary/45" : "bg-primary/30",
        )}
      />

      <div className={cn("absolute inset-x-0 bottom-0 [perspective:600px]", centered ? "h-[46%]" : "h-[62%]")}>
        <div
          className={cn(
