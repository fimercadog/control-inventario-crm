...
export function TeamProfileRow({ member, delay = 0 }: { member: TeamMember; delay?: number }) {
  return (
    <Reveal delay={delay}>
      <div className="grid items-center gap-0 sm:grid-cols-[minmax(0,320px)_1fr]">
        <InitialsAvatar name={member.name} className="aspect-4/5 w-full rounded-2xl shadow-elevation-3" />
        <div className="relative z-10 -mt-6 rounded-2xl bg-card p-7 shadow-elevation-4 sm:-ml-10 sm:mt-0">
