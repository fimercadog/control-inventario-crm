...
const KEY_META: Record<string, { label: string; icon: LucideIcon; tone: string }> = {
  orders_month: { label: "Ventas & Confirmaciones (Mes)", icon: ShoppingCart, tone: REPORT_TONES.indigo },
  revenue_month: { label: "Ingresos por Ventas (Mes)", icon: TrendingUp, tone: REPORT_TONES.emerald },
  ...
  total_products: { label: "Catálogo / Ítems Totales", icon: Boxes, tone: REPORT_TONES.violet },
  low_stock: { label: "Ítems con Stock Bajo", icon: Warehouse, tone: REPORT_TONES.amber },
  pending_purchase_orders: { label: "Órdenes Pendientes", icon: Package, tone: REPORT_TONES.sky },
  total: { label: "Clientes / Solicitantes Totales", icon: Users, tone: REPORT_TONES.cyan },
  active: { label: "Clientes Activos", icon: Users, tone: REPORT_TONES.emerald },
};
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-8">
      ...
      <motion.div variants={itemVariants} className="flex flex-col justify-between gap-2 border-b border-border/60 pb-5 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Visión ejecutiva de ventas, compras, cartera, inventarios y salud financiera del sistema.
          </p>
        </div>
      </motion.div>
      ...
      <motion.section variants={itemVariants} className="space-y-4">
        <ReportSectionHeader title="Ventas & Operaciones Comerciales" description="Rendimiento de facturación, pedidos y volumen de ventas." icon={ShoppingCart} tone={REPORT_TONES.indigo} />
      ...
      {stageChartData.length > 0 ? (
        <motion.section variants={itemVariants} className="grid gap-6 lg:grid-cols-2">
          <ReportChartFrame title="Pipeline Comercial por Etapa" description="Distribución de oportunidades en el embudo comercial." icon={Briefcase} tone={REPORT_TONES.sky}>
          ...
          <ReportDistributionCard
            title="Inventario Con Mayor Disponibilidad"
            description="Top 5 productos con mayor volumen de stock."
            icon={Boxes}
      ...
      {report.purchases ? (
        <motion.section variants={itemVariants} className="space-y-4">
          <ReportSectionHeader title="Compras & Abastecimiento" description="Recepción de inventario y compras a proveedores." icon={Truck} tone={REPORT_TONES.amber} />
      ...
      {report.finance ? (
        <motion.section variants={itemVariants} className="space-y-4">
          <ReportSectionHeader title="Finanzas: Cartera, CxP & Caja" description="Estado de saldos en caja, cobros y compromisos con proveedores." icon={TrendingUp} tone={REPORT_TONES.emerald} />
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      {/* Inventario */}
      <motion.section variants={itemVariants} className="space-y-4">
        <ReportSectionHeader title="Gestión de Inventario" description="Control de existencias y alertas de reposición." icon={Warehouse} tone={REPORT_TONES.violet} />
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
